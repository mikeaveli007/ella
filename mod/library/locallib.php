...
/**
 * class to handle exporting an entire library database
 */
class library_full_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        global $DB;
        if (!$this->cm = get_coursemodule_from_id('library', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'library');
        }
        if (!$this->library = $DB->get_record('library', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'library');
        }
        $entries = $DB->get_records('library_entries', array('libraryid' => $this->library->id));
        list($where, $params) = $DB->get_in_or_equal(array_keys($entries));

        $aliases = $DB->get_records_select('library_alias', 'entryid ' . $where, $params);
        $categoryentries = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {library_entries_categories} ec
            JOIN {library_categories} c
        ...
        foreach (array_keys($entries) as $entry) {
            $this->keyedfiles[$entry] = array_merge(
                $fs->get_area_files($context->id, 'mod_library', 'attachment', $entry, "timemodified", false),
                $fs->get_area_files($context->id, 'mod_library', 'entry', $entry, "timemodified", false)
            );
    ...
    public function prepare_package() {
        ...
        if ($this->get('exporter')->get('formatclass') == PORTFOLIO_FORMAT_SPREADSHEET) {
            $csv = library_generate_export_csv($entries, $aliases, $categories);
            ...
            foreach ($entries as $e) {
                $content = library_entry_portfolio_caller::entry_content(
                    $this->course,
                    $this->cm,
                    $this->library,
                    $e,
                ...
                $entry = new portfolio_format_leap2a_entry('libraryentry' . $e->id, $e->concept, 'entry', $content);
                ...
                if (!empty($this->keyedfiles[$e->id])) {
                    $writer->link_files($entry, $this->keyedfiles[$e->id], 'libraryentry' . $e->id . 'file');
            ...
            $selection = new portfolio_format_leap2a_entry('wholelibrary' . $this->library->id, get_string('modulename', 'library'), 'selection');
    ...
    public function check_permissions() {
        return has_capability('mod/library:export', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'library');
    }
...
/**
 * class to export a single library entry
 *
 * @package   mod_library
 * @copyright 1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class library_entry_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        ...
        if ($this->entryid) {
            if (!$this->entry = $DB->get_record('library_entries', array('id' => $this->entryid))) {
                throw new portfolio_caller_exception('noentry', 'library');
            }
            // in case we don't have USER this will make the entry be printed
            $this->entry->approved = true;
        }
        $this->categories = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {library_entries_categories} ec
            JOIN {library_categories} c
            ON c.id = ec.categoryid
            WHERE ec.entryid = ?', array($this->entryid));
        $context = context_module::instance($this->cm->id);
        if ($this->entry->sourcelibraryid == $this->cm->instance) {
            if ($maincm = get_coursemodule_from_instance('library', $this->entry->libraryid)) {
                $context = context_module::instance($maincm->id);
            }
        }
        $this->aliases = $DB->get_record('library_alias', array('entryid'=>$this->entryid));
        $fs = get_file_storage();
        $this->multifiles = array_merge(
            $fs->get_area_files($context->id, 'mod_library', 'attachment', $this->entry->id, "timemodified", false),
            $fs->get_area_files($context->id, 'mod_library', 'entry', $this->entry->id, "timemodified", false)
        );
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cm->id);
        return has_capability('mod/library:exportentry', $context)
            || ($this->entry->userid == $this->user->id && has_capability('mod/library:exportownentry', $context));
    }
    ...
    public function prepare_package() {
        ...
        $format = $this->exporter->get('format');
        $content = self::entry_content($this->course, $this->cm, $this->library, $this->entry, $this->aliases, $format);
        ...
        if ($this->exporter->get('formatclass') === PORTFOLIO_FORMAT_PLAINHTML) {
            ...
            $entry = new portfolio_format_leap2a_entry('libraryentry' . $this->entry->id, $this->entry->concept, 'entry', $content);
            ...
            throw new portfolio_caller_exception('unexpected_format_class', 'library');
        }
    }
     ...
     * for both this class and the full library exporter
     ...
     * @return string
     */
    public static function entry_content($course, $cm, $library, $entry, $aliases, $format) {
        global $OUTPUT, $DB;
        $entry = clone $entry;
        ...
        $output = '<table class="librarypost dictionary" cellspacing="0">' . "\n";
        ...
        $output .= portfolio_rewrite_pluginfile_urls($entry->definition, $context->id, 'mod_library', 'entry', $entry->id, $format);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class library_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        $sql = 'SELECT DISTINCT f.itemid, ge.concept
                  ...
                  JOIN {library_entries} ge ON (ge.libraryid = g.id AND ge.id = f.itemid)
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_library', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
...
/**
 * Returns library entries tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_library/library_entries to search for library entries
 ...
 */
function mod_library_get_tagged_entries($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    global $OUTPUT;
    $perpage = $exclusivemode ? 20 : 5;

    // Build the SQL query.
    $ctxselect = context_helper::get_preload_record_columns_sql('ctx');
    $query = "SELECT ge.id, ge.concept, ge.libraryid, ge.approved, ge.userid,
                    cm.id AS cmid, c.id AS courseid, c.shortname, c.fullname, $ctxselect
                FROM {library_entries} ge
                JOIN {library} g ON g.id = ge.libraryid
    ...
    $params = array('itemtype' => 'library_entries', 'tagid' => $tag->id, 'component' => 'mod_library',
    ...
    $builder = new core_tag_index_builder('mod_library', 'library_entries', $query, $params, $page * $perpage, $perpage + 1);
            ...
            if ($taggeditem->courseid == $courseid) {
                ...
                if (($cm = $modinfo->get_cm($taggeditem->cmid)) && $cm->uservisible) {
                    if ($taggeditem->approved) {
                        ...
                        $accessible = has_capability('mod/library:approve', context_module::instance($cm->id));
                    }
                }
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new moodle_url('/mod/library/showentry.php', array('eid' => $item->id, 'displayformat' => 'dictionary'));
            ...
            $approved = "";
            if (!$item->approved) {
                $approved = '<br>'. html_writer::span(get_string('entrynotapproved', 'mod_library'), 'badge badge-warning');
            }
        ...
        return new core_tag\output\tagindex($tag, 'mod_library', 'library_entries', $content,
