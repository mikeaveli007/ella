 ...
 * @since      Moodle 3.1
 */
class mod_library_entry_query_builder {

    /** Alias for table library_alias. */
    const ALIAS_ALIAS = 'ga';
    /** Alias for table library_categories. */
    const ALIAS_CATEGORIES = 'gc';
    /** Alias for table library_entries_categories. */
    const ALIAS_ENTRIES_CATEGORIES = 'gec';
    /** Alias for table library_entries. */
    const ALIAS_ENTRIES = 'ge';
    /** Alias for table user. */
    const ALIAS_USER = 'u';
    /** Alias for table library_entries_types. */
    const ALIAS_TYPE = 't';
    ...
    protected $from;
    /** @var object The library we are fetching from. */
    protected $library;
    ...
    public function __construct($library = null) {
        $this->from = sprintf('FROM {library_entries} %s', self::ALIAS_ENTRIES);
        if (!empty($library)) {
            $this->library = $library;
            $this->where[] = sprintf('(%s.libraryid = :gid OR %s.sourcelibraryid = :gid2)',
                self::ALIAS_ENTRIES, self::ALIAS_ENTRIES);
            $this->params['gid'] = $library->id;
            $this->params['gid2'] = $library->id;
        }
    }
     ...
     * @param string $table The table name, without the prefix 'library_'.
    ...
    public function join_alias() {
        $this->joins[] = sprintf('LEFT JOIN {library_alias} %s ON %s = %s',
    ...
    public function join_category($categoryid) {
        ...
        if ($categoryid === LIBRARY_SHOW_ALL_CATEGORIES) {
            $this->joins[] = sprintf('JOIN {library_entries_categories} %s ON %s = %s',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'));

            $this->joins[] = sprintf('JOIN {library_categories} %s ON %s = %s',
            ...
            $this->joins[] = sprintf('LEFT JOIN {library_entries_categories} %s ON %s = %s',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'));

        } else {
            $this->joins[] = sprintf('JOIN {library_entries_categories} %s ON %s = %s AND %s = :joincategoryid',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'),
                self::resolve_field('categoryid', 'entries_categories'));
     ...
     * @param bool $strict When strict uses a JOIN rather than a LEFT JOIN.
     */
    public function join_type($strict = false) {
        $join = $strict ? 'JOIN' : 'LEFT JOIN';
        $this->joins[] = sprintf("$join {library_entries_types} %s ON %s = %s",
            self::ALIAS_TYPE, self::resolve_field('id', 'type'), self::resolve_field('typeid', 'entries'));
    }
