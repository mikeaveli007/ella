...
class entry extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required data for indexing library entries.
     *
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        $sql = "SELECT ge.*, g.course FROM {library_entries} ge
    ...
    /**
     * Returns the documents associated with this library entry id.
     *
     * @param stdClass $entry library entry.
     * @param array    $options
    ...
    public function get_document($entry, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('library', $entry->libraryid, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_library ' . $entry->id . ' document, not all required data is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_library' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $cminfo = $this->get_cm('library', $entry->libraryid, $entry->course);
        } catch (\dml_missing_record_exception $ex) {
        ...
        if (!library_can_view_entry($entry, $cminfo)) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/library/view.php', $docparams);
    }
    ...
    public function get_context_url(\core_search\document $doc) {
        $contextmodule = \context::instance_by_id($doc->get('contextid'));
        return new \moodle_url('/mod/library/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified library entry checking the internal cache.
     *
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriesdata[$entryid])) {
            $this->entriesdata[$entryid] = $DB->get_record_sql("SELECT ge.*, g.course, g.defaultapproval FROM {library_entries} ge
                                                                  JOIN {library} g ON g.id = ge.libraryid
