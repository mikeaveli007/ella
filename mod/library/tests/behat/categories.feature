@mod @mod_library
Feature: Library entries can be organised in categories
  In order to organise library entries
  As a teacher
  I need to be able to create, edit and delete categories

  @javascript
  Scenario: Library entries can be organised in categories and categories can be autolinked
    ...
    And the following "activities" exist:
      ...
      | library | MyLibrary | Test library description | encyclopedia  | C1     | library1 |
    And the following "activities" exist:
      | activity | name       | intro                                                           | course | idnumber  |
      | label    | name       | check autolinking of CategoryAutoLinks and CategoryNoLinks text | C1     | label1    |
    And the "library" filter is "on"
# Log in as a teacher and make sure nothing is yet autolinked
    ...
    And "a.library.autolink" "css_element" should not exist
# Create, edit and delete categories
    And I follow "MyLibrary"
    ...
    And I press "Back"
# Add library entries in categories and outside
    And I add a library entry with the following data:
...
# Check that category is autolinked from the text in the course
    ...
    And I should see "CategoryAutoLinks" in the "a.library.autolink" "css_element"
    And I should see "CategoryNoLinks"
