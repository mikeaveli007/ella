...
class mod_library_external_testcase extends externallib_advanced_testcase {
    ...
    /**
     * Test get_libraries_by_courses
     */
    public function test_get_libraries_by_courses() {
        ...
        $g1 = self::getDataGenerator()->create_module('library', array('course' => $c1->id, 'name' => 'First Library'));
        $g2 = self::getDataGenerator()->create_module('library', array('course' => $c1->id, 'name' => 'Second Library'));
        $g3 = self::getDataGenerator()->create_module('library', array('course' => $c2->id, 'name' => 'Third Library'));
        ...
        $this->setUser($s1);
        $libraries = mod_library_external::get_libraries_by_courses(array());
        $libraries = external_api::clean_returnvalue(mod_library_external::get_libraries_by_courses_returns(), $libraries);

        $this->assertCount(2, $libraries['libraries']);
        $this->assertEquals('First Library', $libraries['libraries'][0]['name']);
        $this->assertEquals('Second Library', $libraries['libraries'][1]['name']);
        $this->assertEquals(1, $libraries['libraries'][0]['canaddentry']);
        $this->assertEquals(1, $libraries['libraries'][1]['canaddentry']);

        // Check results with specific course IDs.
        $libraries = mod_library_external::get_libraries_by_courses(array($c1->id, $c2->id));
        $libraries = external_api::clean_returnvalue(mod_library_external::get_libraries_by_courses_returns(), $libraries);
        ...
        $this->assertEquals('course', $libraries['warnings'][0]['item']);
        $this->assertEquals($c2->id, $libraries['warnings'][0]['itemid']);
        $this->assertEquals('1', $libraries['warnings'][0]['warningcode']);
        $this->assertEquals(1, $libraries['libraries'][0]['canaddentry']);

        // Now as admin.
        $this->setAdminUser();

        $libraries = mod_library_external::get_libraries_by_courses(array($c2->id));
        $libraries = external_api::clean_returnvalue(mod_library_external::get_libraries_by_courses_returns(), $libraries);

        $this->assertCount(1, $libraries['libraries']);
        $this->assertEquals('Third Library', $libraries['libraries'][0]['name']);
        $this->assertEquals(1, $libraries['libraries'][0]['canaddentry']);
    }
    ...
    public function test_view_library() {
        ...
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $u1 = $this->getDataGenerator()->create_user();
        $this->getDataGenerator()->enrol_user($u1->id, $c1->id);

        $sink = $this->redirectEvents();
        $this->setUser($u1);
        $return = mod_library_external::view_library($g1->id, 'letter');
        $return = external_api::clean_returnvalue(mod_library_external::view_library_returns(), $return);
        ...
        $this->assertEquals('\mod_library\event\course_module_viewed', $events[0]->eventname);
        $sink->close();
    }
     ...
     */
    public function test_view_library_without_permission() {
        $this->resetAfterTest(true);
        ...
        assign_capability('mod/library:view', CAP_PROHIBIT, $role->id, $ctx, true);
        accesslib_clear_all_caches_for_unit_testing();

        // Assertion.
        $this->setUser($u1);
        mod_library_external::view_library($g1->id, 'letter');
    }
    ...
    public function test_view_entry() {
        ...
        $c1 = $this->getDataGenerator()->create_course();
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $g2 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id, 'visible' => false));
        $u1 = $this->getDataGenerator()->create_user();
        ...
        // Test readable entry.
        $sink = $this->redirectEvents();
        $return = mod_library_external::view_entry($e1->id);
        $return = external_api::clean_returnvalue(mod_library_external::view_entry_returns(), $return);
        $events = $sink->get_events();
        $this->assertTrue($return['status']);
        $this->assertEmpty($return['warnings']);
        $this->assertCount(1, $events);
        $this->assertEquals('\mod_library\event\entry_viewed', $events[0]->eventname);
        $sink->close();

        // Test non-approved of self.
        $return = mod_library_external::view_entry($e2->id);
        $return = external_api::clean_returnvalue(mod_library_external::view_entry_returns(), $return);
        $events = $sink->get_events();
        $this->assertTrue($return['status']);
        $this->assertEmpty($return['warnings']);
        ...
        try {
            mod_library_external::view_entry($e3->id);
        ...
        mod_library_external::view_entry($e4->id);
    }
    ...
    public function test_get_entries_by_letter() {
        ...
        $c1 = $this->getDataGenerator()->create_course();
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $g2 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $u1 = $this->getDataGenerator()->create_user();
        $ctx = context_module::instance($g1->cmid);
        $this->getDataGenerator()->enrol_user($u1->id, $c1->id);
        ...
        $return = mod_library_external::get_entries_by_letter($g1->id, 'ALL', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
        ...
        $return = mod_library_external::get_entries_by_letter($g1->id, 'ALL', 0, 20, array('includenotapproved' => 1));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
    ...
    public function test_get_entries_by_letter_with_parameters() {
        ...
        $return = mod_library_external::get_entries_by_letter($g1->id, 'b', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
        ...
        $return = mod_library_external::get_entries_by_letter($g1->id, 'SPECIAL', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
        ...
        $return = mod_library_external::get_entries_by_letter($g1->id, 'ALL', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a->id, $return['entries'][0]['id']);
        $return = mod_library_external::get_entries_by_letter($g1->id, 'ALL', 1, 2, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_letter_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1c->id, $return['entries'][0]['id']);
    ...
    public function test_get_entries_by_date() {
        ...
        $gg = $this->getDataGenerator()->get_plugin_generator('mod_library');
        $c1 = $this->getDataGenerator()->create_course();
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id, 'displayformat' => 'entrylist'));
        $g2 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $u1 = $this->getDataGenerator()->create_user();
        $ctx = context_module::instance($g1->cmid);
        $this->getDataGenerator()->enrol_user($u1->id, $c1->id);
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'UPDATE', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'UPDATE', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1b->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'CREATION', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'CREATION', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1b->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'CREATION', 'ASC', 0, 20,
        ...
        $return = mod_library_external::get_entries_by_date($g1->id, 'CREATION', 'ASC', 0, 2,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1a->id, $return['entries'][0]['id']);
        $this->assertEquals($e1c->id, $return['entries'][1]['id']);
        $return = mod_library_external::get_entries_by_date($g1->id, 'CREATION', 'ASC', 2, 2,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_date_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
    ...
    public function test_get_categories() {
        ...
        $return = mod_library_external::get_categories($g1->id, 0, 20);
        $return = external_api::clean_returnvalue(mod_library_external::get_categories_returns(), $return);
        ...
        $return = mod_library_external::get_categories($g1->id, 1, 2);
        $return = external_api::clean_returnvalue(mod_library_external::get_categories_returns(), $return);
    ...
    public function test_get_entries_by_category() {
        ...
        $c1 = $this->getDataGenerator()->create_course();
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id, 'displayformat' => 'entrylist'));
        $g2 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id, 'displayformat' => 'entrylist'));
        $u1 = $this->getDataGenerator()->create_user();
        $ctx = context_module::instance($g1->cmid);
        ...
        // Browse one category.
        $return = mod_library_external::get_entries_by_category($g1->id, $cat1a->id, 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        ...
        $return = mod_library_external::get_entries_by_category($g1->id, LIBRARY_SHOW_ALL_CATEGORIES, 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        ...
        $return = mod_library_external::get_entries_by_category($g1->id, LIBRARY_SHOW_NOT_CATEGORISED, 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(1, $return['count']);
        ...
        $return = mod_library_external::get_entries_by_category($g1->id, $cat1b->id, 0, 20,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(2, $return['count']);
        ...
        // Using limit.
        $return = mod_library_external::get_entries_by_category($g1->id, LIBRARY_SHOW_ALL_CATEGORIES, 0, 3,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        $this->assertCount(3, $return['entries']);
        ...
        $return = mod_library_external::get_entries_by_category($g1->id, LIBRARY_SHOW_ALL_CATEGORIES, 3, 2,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_category_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(6, $return['count']);
    ...
    public function test_get_authors() {
        ...
        // Simple request.
        $return = mod_library_external::get_authors($g1->id, 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_authors_returns(), $return);
        ...
        $return = mod_library_external::get_authors($g1->id, 0, 20, array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_authors_returns(), $return);
        ...
        // Pagination.
        $return = mod_library_external::get_authors($g1->id, 1, 1, array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_authors_returns(), $return);
    ...
    public function test_get_entries_by_author() {
        ...
        // Requesting a single letter.
        $return = mod_library_external::get_entries_by_author($g1->id, 'u', 'LASTNAME', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        ...
        // Requesting special letters.
        $return = mod_library_external::get_entries_by_author($g1->id, 'SPECIAL', 'LASTNAME', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(1, $return['count']);
        ...
        // Requesting with limit.
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'LASTNAME', 'ASC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(6, $return['count']);
        $this->assertEquals($e1d1->id, $return['entries'][0]['id']);
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'LASTNAME', 'ASC', 1, 2, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(6, $return['count']);
        ...
        $this->setAdminUser();
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'LASTNAME', 'ASC', 0, 20,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        ...
        // Changing order.
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'LASTNAME', 'DESC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(6, $return['count']);
        $this->assertEquals($e1a1->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'FIRSTNAME', 'ASC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(6, $return['count']);
        $this->assertEquals($e1b2->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author($g1->id, 'ALL', 'FIRSTNAME', 'DESC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(6, $return['count']);
        $this->assertEquals($e1d1->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author($g1->id, 'z', 'FIRSTNAME', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1d1->id, $return['entries'][0]['id']);
    ...
    public function test_get_entries_by_author_id() {
        ...
        // Standard request.
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CONCEPT', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a2->id, $return['entries'][0]['id']);
        $this->assertEquals($e1a3->id, $return['entries'][1]['id']);
        ...
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CONCEPT', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a1->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CREATION', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        ...
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CREATION', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a2->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'UPDATE', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a1->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'UPDATE', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e1a2->id, $return['entries'][0]['id']);
        ...
        // Including non approved.
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CONCEPT', 'ASC', 0, 20,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        ...
        // Pagination.
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CONCEPT', 'ASC', 0, 2,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1a2->id, $return['entries'][0]['id']);
        $this->assertEquals($e1a4->id, $return['entries'][1]['id']);
        $return = mod_library_external::get_entries_by_author_id($g1->id, $u1->id, 'CONCEPT', 'ASC', 1, 2,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_author_id_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
    ...
    public function test_get_entries_by_search() {
        ...
        // Test simple query.
        $query = 'hero';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'CONCEPT', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(1, $return['count']);
        ...
        $query = 'hero';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, true, 'CONCEPT', 'ASC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(2, $return['count']);
        ...
        // Concept descending.
        $query = 'hero';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, true, 'CONCEPT', 'DESC', 0, 20, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(2, $return['count']);
        $this->assertEquals($e3->id, $return['entries'][0]['id']);
        ...
        $query = 'ou';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'CREATION', 'ASC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e7->id, $return['entries'][0]['id']);
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'CREATION', 'DESC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
        ...
        $query = 'ou';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'UPDATE', 'ASC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e2->id, $return['entries'][0]['id']);
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'UPDATE', 'DESC', 0, 1, array());
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
        $this->assertEquals($e7->id, $return['entries'][0]['id']);
        ...
        $query = 'ou';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, false, 'CONCEPT', 'ASC', 0, 20,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        ...
        $query = '+Heroes -Abcd';
        $return = mod_library_external::get_entries_by_search($g1->id, $query, true, 'CONCEPT', 'ASC', 0, 20,
            array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_search_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(2, $return['count']);
    ...
    public function test_get_entries_by_term() {
        ...
        $return = mod_library_external::get_entries_by_term($g1->id, 'cat', 0, 20, array('includenotapproved' => false));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_term_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(2, $return['count']);
        ...
        // Search alias.
        $return = mod_library_external::get_entries_by_term($g1->id, 'dog', 0, 20, array('includenotapproved' => false));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_term_returns(), $return);
        ...
        $return = mod_library_external::get_entries_by_term($g1->id, 'dog', 0, 20, array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_term_returns(), $return);
        $this->assertCount(3, $return['entries']);
        $this->assertEquals(3, $return['count']);
        // Compare ids, ignore ordering of array, using canonicalize parameter of assertEquals.
        ...
        // Pagination.
        $return = mod_library_external::get_entries_by_term($g1->id, 'dog', 0, 1, array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_term_returns(), $return);
        $this->assertCount(1, $return['entries']);
        ...
        $this->assertEquals(3, $return['count']);
        $return = mod_library_external::get_entries_by_term($g1->id, 'dog', 1, 1, array('includenotapproved' => true));
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_by_term_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(3, $return['count']);
    }
    ...
    public function test_get_entries_to_approve() {
        ...
        // Simple listing.
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CONCEPT', 'ASC', 0, 20);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1d->id, $return['entries'][0]['id']);
        $this->assertEquals($e1c->id, $return['entries'][1]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CONCEPT', 'DESC', 0, 20);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(4, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1b->id, $return['entries'][0]['id']);
        ...
        // Filtering by letter.
        $return = mod_library_external::get_entries_to_approve($g1->id, 'a', 'CONCEPT', 'ASC', 0, 20);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(1, $return['count']);
        $this->assertEquals($e1c->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'SPECIAL', 'CONCEPT', 'ASC', 0, 20);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(1, $return['count']);
        $this->assertEquals($e1d->id, $return['entries'][0]['id']);
        ...
        // Pagination.
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CONCEPT', 'ASC', 0, 2);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1d->id, $return['entries'][0]['id']);
        $this->assertEquals($e1c->id, $return['entries'][1]['id']);
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CONCEPT', 'ASC', 1, 2);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(2, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1c->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CREATION', 'ASC', 0, 1);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1b->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CREATION', 'DESC', 0, 1);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1a->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'UPDATE', 'ASC', 0, 1);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1c->id, $return['entries'][0]['id']);
        ...
        $return = mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'UPDATE', 'DESC', 0, 1);
        $return = external_api::clean_returnvalue(mod_library_external::get_entries_to_approve_returns(), $return);
        $this->assertCount(1, $return['entries']);
        $this->assertEquals(4, $return['count']);
        $this->assertEquals($e1d->id, $return['entries'][0]['id']);
        ...
        mod_library_external::get_entries_to_approve($g1->id, 'ALL', 'CONCEPT', 'ASC', 0, 1);
    ...
    public function test_get_entry_by_id() {
        ...
        $g1 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id));
        $g2 = $this->getDataGenerator()->create_module('library', array('course' => $c1->id, 'visible' => 0));
        $u1 = $this->getDataGenerator()->create_user();
        ...
        $this->setUser($u1);
        $return = mod_library_external::get_entry_by_id($e1->id);
        $return = external_api::clean_returnvalue(mod_library_external::get_entry_by_id_returns(), $return);
        ...
        $return = mod_library_external::get_entry_by_id($e2->id);
        $return = external_api::clean_returnvalue(mod_library_external::get_entry_by_id_returns(), $return);
        $this->assertEquals($e2->id, $return['entry']['id']);

        try {
            $return = mod_library_external::get_entry_by_id($e3->id);
        ...
        try {
            $return = mod_library_external::get_entry_by_id($e4->id);
    ...
    public function test_add_entry_without_optional_settings() {
        ...
        $return = mod_library_external::add_entry($library->id, $concept, $definition, FORMAT_HTML);
        $return = external_api::clean_returnvalue(mod_library_external::add_entry_returns(), $return);

        // Get entry from DB.
        $entry = $DB->get_record('library_entries', array('id' => $return['entryid']));

        $this->assertEquals($concept, $entry->concept);
        $this->assertEquals($definition, $entry->definition);
        $this->assertEquals($CFG->library_linkentries, $entry->usedynalink);
        $this->assertEquals($CFG->library_casesensitive, $entry->casesensitive);
        $this->assertEquals($CFG->library_fullmatch, $entry->fullmatch);
        $this->assertEmpty($DB->get_records('library_alias', array('entryid' => $return['entryid'])));
        $this->assertEmpty($DB->get_records('library_entries_categories', array('entryid' => $return['entryid'])));
    }
    ...
    public function test_add_entry_with_aliases() {
        ...
        $return = mod_library_external::add_entry($library->id, $concept, $definition, FORMAT_HTML, $options);
        $return = external_api::clean_returnvalue(mod_library_external::add_entry_returns(), $return);

        $aliases = $DB->get_records('library_alias', array('entryid' => $return['entryid']));
    ...
    public function test_add_entry_in_categories() {
        ...
        $library = $this->getDataGenerator()->create_module('library', array('course' => $course->id));
        $gg = $this->getDataGenerator()->get_plugin_generator('mod_library');
        $cat1 = $gg->create_category($library);
        $cat2 = $gg->create_category($library);
        ...
        $categories = $DB->get_records('library_entries_categories', array('entryid' => $return['entryid']));
    ...
    public function test_add_entry_with_attachments() {
        ...
        $editorfiles = external_util::get_area_files($context->id, 'mod_library', 'entry', $return['entryid']);
        $attachmentfiles = external_util::get_area_files($context->id, 'mod_library', 'attachment', $return['entryid']);
    ...
    public function test_get_entry_rating_information() {
        ...
        // Create the library and contents.
        $record = new stdClass();
        ...
        $library = $this->getDataGenerator()->create_module('library', $record);
        $context = context_module::instance($library->cmid);

        $gg = $this->getDataGenerator()->get_plugin_generator('mod_library');
        $entry = $gg->create_content($library, array('approved' => 1, 'userid' => $user1->id));

        // Rate the entry as user2.
        $rating1 = new stdClass();
        $rating1->contextid = $context->id;
        $rating1->component = 'mod_library';
        ...
        $this->setUser($user1);
        $result = mod_library_external::get_entry_by_id($entry->id);
        $result = external_api::clean_returnvalue(mod_library_external::get_entry_by_id_returns(), $result);
